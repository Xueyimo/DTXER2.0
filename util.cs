...
namespace arcball
{
    class util
    {
        ...
        public PO GIsXl(PO s1, PO d1, PO s2, PO d2)
        {
            /*
             * 求出边s1-d1与s2-d2的交点，并返回相应参数
             */
            float D, dl1, dl2,f1,f2;
            PO xpo = new PO();
            D = (s1.x - d1.x) * (d2.y - s2.y) - (d2.x - s2.x) * (s1.y - d1.y);
            dl1 = (d2.x - d1.x) * (d2.y - s2.y) - (d2.x - s2.x) * (d2.y - d1.y);
            dl2 = (s1.x - d1.x) * (d2.y - d1.y) - (d2.x - d1.x) * (s1.y - d1.y);
        ...
        //实现边界检测的函数
        public float Edge_Test_Fun(PO p1,PO p2,PO p0)
        {
            /*
             * p0是待检测点，p1-p2是构成封闭边界的一条边
             */
            float a, b, a1, b1, c1, m0, k1, k3, alf1, alf2;
            if (p1.x == p2.x)
            {
                k1 = p1.x - p0.x;
                k3 = (p1.x - p0.x) * (p1.x - p0.x);
                alf1 = (float)System.Math.Atan((p1.y - p0.y) / System.Math.Pow(k3, 0.5));
                alf2 = (float)System.Math.Atan((p2.y- p0.y) / System.Math.Pow(k3, 0.5));
                return (float)(k1 * (alf2 - alf1) / System.Math.Pow(k3, 0.5));
            }
            else if (p1.y == p2.y)
            {
                k1 = -(p1.y - p0.y);
                k3 = (p1.y - p0.y) * (p1.y - p0.y);
                alf1 = (float)System.Math.Atan((p1.x - p0.x) / System.Math.Pow(k3, 0.5));
                alf2 = (float)System.Math.Atan((p2.x - p0.x) / System.Math.Pow(k3, 0.5));
                return (float)(k1 * (alf2 - alf1) / System.Math.Pow(k3, 0.5));
            }
            ...
            m0=-b+p0.y-a*p0.x;
            a1=a*a+1;
            b1=-2*p0.x+2*a*(b-p0.y);
            c1=p0.x*p0.x+(b-p0.y)*(b-p0.y);
            k1 = m0/a1;
            k3 = (c1 - a1 * b1 * b1 / (4 * a1 * a1)) / a1;
            alf1 = (float)System.Math.Atan((p1.x + b1 / (2 * a1)) / System.Math.Pow(k3, 0.5));
            alf2 = (float)System.Math.Atan((p2.x + b1 / (2 * a1))/System.Math.Pow(k3, 0.5));
            return (float)(k1 * (alf2 - alf1) / System.Math.Pow(k3,0.5));
        }
